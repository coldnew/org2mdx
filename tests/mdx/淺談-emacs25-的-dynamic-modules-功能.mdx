...
date: "2016-01-03T02:42:00.000Z"
updated: "2016-01-03T02:42:00.000Z"
abbrlink: 2d16cc25
tags:
  - emacs
alias:
  - blog/2015/01/03_emacsdynamic/index.html
...
在 emacs-lisp 中，除了使用 [defun](http://www.gnu.org/software/emacs/manual/html_node/elisp/Defining-Functions.html) 以外，我們也可以使用 [fset](http://www.gnu.org/software/emacs/manual/html_node/elisp/Defining-Functions.html) 搭配匿名函式來完成我 們要使用的 function。
...
為了確認我們編譯出來的 .so 模組是否有正確執行，我們另外弄一個 `test.el` 來執行 [ert](https://www.gnu.org/software/emacs/manual/html_mono/ert.html) 測試，因此先來完成這個測試程式。

## test.el

在 test.el 中我們載入了 [ert](https://www.gnu.org/software/emacs/manual/html_mono/ert.html) 模組以及我們即將要編譯出來的 `hello-core.so` 模組，注意到載入 dynamic module 模組的方式和載入一般 .el 的函式庫是一樣的。
...
這邊可以參考 emacs 程式碼中的 [src/modules/mod-test/mod-test.c](https://github.com/jwiegley/emacs/blob/54617ada51e49af04f0c3f609e724aafcaf0df16/modules/mod-test/mod-test.c) ，先加入這樣的實現
...
好像還是用 C 寫的效能比較好？我們來看看一個反例，用 C++ 寫的模仿 [s.el](https://github.com/magnars/s.el) 的功能
...
目前我測試用的程式碼已經放到 [GitHub](https://github.com/coldnew/emacs-module-test) 上，有興趣也歡迎來玩玩看，看能不能替 emacs 增加更多有趣的功能。

# 延伸閱讀

`[1]` [ejectで學ぶ Dynamic module 機能](http://syohex.hatenablog.com/entry/2015/12/16/185458)

`[2]` [Emacsから Cの関數をつかう -> はやい](http://qiita.com/syohex/items/5e06a34f1426fd2f28d8#_reference-c55c6773db283acc422d)

`[3]` [Emacsに mrubyを組み込んでみた](http://qiita.com/syohex/items/fa71f1c61107b5e30c43)

`[4]` [https://lists.gnu.org/archive/html/emacs-devel/2015-02/msg00960.html](https://lists.gnu.org/archive/html/emacs-devel/2015-02/msg00960.html)