...
在 [使用 ClojureScript 來寫 node.js 程式](http://coldnew.github.io/blog/2013/06/29_82531.html) 一文當中，我們提到了可以用
ClojureScript 來寫 node.js 的程式，可是如果要引用 node.js 的外部函式，比
如說 [tty.js](https://github.com/chjj/tty.js) 時，要怎麼辦呢？

[tty.js](https://github.com/chjj/tty.js) 是一個可以讓你在瀏覽器下使用終端機的一個很酷的 node.js 程式，雖
...
此外，這邊 [Google Closure Compiler](https://developers.google.com/closure/compiler/?hl=zh-TW) 的最佳化設定設定成為 **Simple** ，
...
<div className="row show-grid"><div className="col-md-6"></div></div>
**javascript**
...
也需要載入 [node-clojurescript](https://github.com/michaelsbradleyjr/node-clojurescript) 的函式庫，你不需要安裝他，因為這步驟
cljsbuild 已經幫你做好了。
...
在 [使用 ClojureScript 來寫 node.js 程式](http://coldnew.github.io/blog/2013/06/29_82531.html) 一文說過要怎樣編譯我們的
...
執行完後使用瀏覽器瀏覽 [http://localhost:8080](http://localhost:8080) 這個
位址，並輸入帳號: `foo` 密碼: `bar` ，
多打開幾個終端機，執行 htop、vim 等程式，你就可以看到如下圖的狀況。

![讓-Clojurescript-使用-node.js-的外部函式庫-以-tty/ttyjs.png](讓-Clojurescript-使用-node.js-的外部函式庫-以-tty/ttyjs.png)
