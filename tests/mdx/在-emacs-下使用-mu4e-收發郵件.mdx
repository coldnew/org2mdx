...
date: "2016-01-02T16:22:00.000Z"
updated: "2016-01-02T16:22:00.000Z"
abbrlink: 6a7aa5c1
category:
  - emacs 套件介紹
tags:
  - emacs
alias:
  - blog/2016/01-02_mu4e/index.html
description: Generated from Org-mode
---

最近又重新改寫了我的 emacs 設定，捨棄掉了 [spacemacs](https://github.com/syl20bnr/spacemacs) 並重新使用 [org-mode](http://orgmode.org/) 撰寫，你可以在 [這裡](http://coldnew.github.io/coldnew-emacs) 看到我目前的設定。

既然剛好重寫，就順便紀錄一下如何在 emacs 下進行郵件的收發，在 emacs 下我們有很多種收發電子郵件的選擇，比如 [gnus](http://www.emacswiki.org/emacs/GnusGmail) 、[wanderlust](http://www.emacswiki.org/emacs/WanderLust) ，本文要說的是另外一個很多人在用並且比較好設定的 [mu4e](http://www.djcbsoftware.nl/code/mu/mu4e.html)。
...
[mu4e](http://www.djcbsoftware.nl/code/mu/mu4e.html) 實際上是 [mu](http://www.djcbsoftware.nl/code/mu/mu4e.html) 的 emacs 客戶端，因此我們真正要安裝的是 [mu](http://www.djcbsoftware.nl/code/mu/mu4e.html) 這套軟體。
...
[mu](http://www.djcbsoftware.nl/code/mu/mu4e.html) 只負責進行索引，並未內建擷取信件的工具，因此你需要安裝第三方程式來幫忙下載郵件到本地端，常用的有 [offlineimap](http://offlineimap.org/) 以及 [mbsync](http://isync.sourceforge.net/) ，在本文中使用 offlinemap 來負責擷取信件。
...
首先連線到 [https://support.google.com/mail/answer/185833?hl=en\&rd=1](https://support.google.com/mail/answer/185833?hl=en\&rd=1) 該頁面描述了產生 App password 的方法

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/a1.png)

接著就去進行你的兩步驟驗證，完成後你就可以在個人的設定頁面去設定 `應用程式密碼` ，首先我們選擇其他應用程式

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/a2.png)

接著設定應用程式名稱為 `emacs`

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/a3.png)

完成以後你就會得到一組 `16 字元` 的密碼，我們將他填到 `~/.authinfo.gpg` 的 `YOUR_PASSWORD` 欄位去。

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/a4.png)
...
接下來就是我們 mu4e 的設定，我們設定了信件存放位址為 `~/Maildir` 這個資料夾，並且使用 [offlineimap](http://offlineimap.org/) 進行信件的擷取功能，每 `60 秒` 更新一次資訊，值得一提的是由於 [offlineimap](http://offlineimap.org/) 是外部程式，在擷取信件的時後並不會卡住你的 emacs。
...
在我們收信的時候，有些信件可能是 HTML 的格式，因此我們使用 emacs 內建的 `eww` 將其轉換為純文字檔好方便閱讀，並用一些 [建議](http://www.djcbsoftware.nl/code/mu/mu4e/Displaying-rich_002dtext-messages.html) 的按鍵設定
...
第一次進行使用的時候，我們要先透過 [offlineimap](http://offlineimap.org/) 來收取 Gmail 上的郵件，於是在另外一個終端機上手動執行 offlineimap 這個程式，如果你仿照我的設定，你會看到執行完 offlineimap 後，emacs 會來相你詢問 `~/.authinfo.gpg` 的密碼，提供給他後 offlineimap 就開始下載郵件了
...
![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/b1.png)

接下來輸入 `j` 切換要瀏覽的信件群組

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/b2.png)

然後對你有興趣的信件按下 `ENTER` 後就可以看到內文囉\~

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/b3.png)

更多的操作就不在這邊贅述了，請參考 [mu4e Manual](http://www.djcbsoftware.nl/code/mu/mu4e/index.html) 。

# 在 emacs 下寄送信件

會收信了以後，讓我們來看看怎樣發送信件，前面說到了 mu4e 實際上是透過 emacs 內建的功能來寄送信件，因此我們只要呼叫 [compose-mail](http://www.gnu.org/software/emacs/manual/html_node/emacs/Sending-Mail.html) 這個命令 (`C-x m`) 即可進入寫信件的視窗

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/c1.png)

填寫完成你的訊息後，只要輸入 `C-c C-c` 就會將信件寄送出去囉，這邊是我在 gmail 收到的結果

![img](%E5%9C%A8-emacs-%E4%B8%8B%E4%BD%BF%E7%94%A8-mu4e-%E6%94%B6%E7%99%BC%E9%83%B5%E4%BB%B6/c2.png)
...
`[1]` [http://www.djcbsoftware.nl/code/mu/mu4e.html](http://www.djcbsoftware.nl/code/mu/mu4e.html)

`[2]` [Offlineimap with SSL files on OSX](http://rudolfochrist.github.io/blog/2015/03/21/offlineimap-with-ssl-files-on-osx/)

`[3]` [offlineimap stops working after update to 6.6.0](https://bbs.archlinux.org/viewtopic.php?pid=1588425#p1588425)

`[4]` [Manage your gmail account in emacs with mu4e](https://gist.github.com/areina/3879626)

`[5]` [2-step authentication for IMap?](https://productforums.google.com/forum/#!topic/gmail/WUaXHSdI3WM)